# FIXME: the catch_errors and catch_warnings should be coming from package settings ideally from a debug flag
# FIXME: should we be including codumenation? probabaly not
...
try_catch_cnds <- function(expr, error_value = NULL, catch_errors = TRUE, catch_warnings = TRUE, truncate_call_stack = TRUE, env = rlang::caller_env()) {
    ...
    if (truncate_call_stack && is(cnd$trace, "rlang_trace")) {
      ...
      if (any(is_helper_start) && any(is_helper_end[cumsum(is_helper_start) > 0])) {
        ...
        for(first_call in which(is_helper_start)) {
          last_call <- first_call + which(is_helper_end[seq_along(is_helper_end) > first_call])[1] 
          if (last_call > first_call) {
            ...
            cnd$trace <- 
              ...
              dplyr::mutate(
                parent = ifelse(.data$parent > last_call - total_shift, 
                                .data$parent - shift, 
                                .data$parent)
              ) |>
              # omit the helper callstack
              dplyr::filter(dplyr::row_number() < first_call - total_shift | 
  ...
  if (catch_warnings)
    result <- tryCatch(error = handle_error, withCallingHandlers(
      warning = handle_warning, expr
    ))
  else 
...
format_cnds <- function(conditions, include_symbol = TRUE, include_call = TRUE, prefix = "", call_prefix = "in ", indent = 0) {
  if (nrow(conditions) == 0L) return(c())
...
show_cnds <- function(conditions, include_summary = TRUE, summary_message = NULL, include_call = include_summary, indent_cnds = include_summary, include_cnd_calls = TRUE, call = rlang::caller_call()) {
  ...
  if (has_summary && nrow(conditions) == 1L) {
    ...
    summary_line <- paste(
      ...
      "{symbol$arrow_right}",
      format_cnds(
        conditions, include_call = include_cnd_calls, include_symbol = FALSE, 
        call_prefix = ""
      )
    )
    formatted_cnds <- c()
    ...
    formatted_cnds <- conditions |> 
      format_cnds(include_call = include_cnd_calls, indent = indent_cnds,
  ...
  if (has_summary) {
    summary_icon <- 
      if(nrow(conditions) == 0L) "v"
    else if (any(conditions$type == "error")) "x"
    else "!"
