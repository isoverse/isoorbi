...
orbi_get_problems <- function(obj) {
  stopifnot(
    "`obj` must be a list or data frame" = !missing(obj) && (is.list(obj) || is.data.frame(obj))
  )
  
  # template
  probs <- tibble(uid = factor(), type = character(), call = character(), message = character(), condition = list())
  ...
  if ("problems" %in% names(obj)) {
    ...
    if (!is.data.frame(probs) && is.list(probs)) new_probs <- dplyr::bind_rows(new_probs)
