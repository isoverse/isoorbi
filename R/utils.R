...
#'@examples
#' fpath <- system.file("extdata", "testfile_Flow_Exploration_small.isox", package="isoorbi")
#' df <- orbi_read_isox(filepath = fpath) %>% orbi_simplify_isox() %>% orbi_filter_satellitePeaks()
#'
...
#' df <- df %>% orbi_filter_TICxIT(truncate_extreme = 1)
#'
#' @return Filtered data frame (tibble)
#' @export
...
#' @param filenames Vector of filen ames to select
...
#' fpath <- system.file("extdata", "testfile_Flow_Exploration_small.isox", package="isoorbi")
#' df <- orbi_read_isox(filepath = fpath) %>% orbi_simplify_isox()
#' df <- df %>% orbi_filter_isox(isotopocules=c("M0"),filenames=c("s3744"),compounds="HSO4-", time_min=0,time_max=1)
#'
#' @return  Filtered data frame (tibble)
#' @export
orbi_filter_isox <- function(dataset, isotopocules, filenames, compounds, time_min, time_max) {
...
#' fpath <- system.file("extdata", "testfile_Flow_Exploration_small.isox", package="isoorbi")
#' df <- orbi_read_isox(filepath = fpath) %>% orbi_simplify_isox()
#' df <- df %>% orbi_basepeak(basepeak = "M0")
#'
#'
