...
test_that("try_catch_cnds() works", {
  ...
  expect_error(try_catch_cnds(wrap_f(), catch_errors = FALSE), "error") |>
    suppressMessages()
  expect_warning(out2 <- try_catch_cnds(wrap_f(), catch_warnings = FALSE, error_value = FALSE), "trouble") |>
    suppressWarnings() |> suppressMessages()
  expect_equal(out2$result, FALSE)
  
  # resulting messages
  expect_snapshot(show_cnds(out$conditions))
  expect_snapshot(show_cnds(out$conditions, include_call = FALSE, summary_message = "hello {.pkg isoorbi}"))
  expect_snapshot(show_cnds(out$conditions, include_summary = FALSE, include_cnd_calls = FALSE))
