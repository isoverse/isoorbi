...
test_that("orbi_export_data_to_excel() tests", {
  ...
  expect_error(
    orbi_export_data_to_excel(df, 42),
    "`file` needs to be a filepath",
    fixed = TRUE
  )

  expect_error(
    orbi_export_data_to_excel(df, "c", "c"),
  ...
  expect_error(
    orbi_export_data_to_excel(df, "c", 2, int_format = TRUE),
  ...
  tmp_file <- tempfile()
  expect_type(
    orbi_export_data_to_excel(df, tmp_file, 2, int_format = "0"),
    "list"
  ) |>
    suppressMessages()
  unlink(tmp_file)
