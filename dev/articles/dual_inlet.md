...
``` fansi
✔ [306ms] orbi_aggregate_raw() aggregated file_info (1), scans (12.34k), peaks
(184.77k), and spectra (277) from 1 file using the standard aggregator
```
...
``` fansi
✔ [2s] orbi_identify_isotopocules() identified 49.35k/184.77k peaks (27%)
...
``` fansi
✔ [1.1s] orbi_flag_satellite_peaks() confirmed there are no satellite peaks
```
